...
works the same way. An agent driving this codex exec pipeline on the operator's behalf
relays these gates verbatim and waits; it does not approve them itself.
...
changes. When the circled sparks do not cover all three angle types, the insight `codex exec`
call extends or branches a circled lineage to fill the shortfall and never introduces an
uncircled idea. Each backfilled variant carries the optional in-memory field
